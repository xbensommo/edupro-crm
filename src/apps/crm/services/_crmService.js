...
export function createCrmService(store = useAppStore()) {
  const actions = Object.freeze(
    Object.fromEntries(
      Object.values(CRM_COLLECTIONS).map((collectionName) => [
        collectionName,
        requireCollectionActions(store, collectionName),
      ]),
    ),
  );
  ...
  async function fetchCollection(collectionName, params = {}) {
    ...
    await collectionActions.fetchInitialPage(params);
  ...
  /**
   * Engagements Functions starts from here
   */
  ...
  function clientPrimaryLabel({data}) {
    const fullName = [data?.firstName, data?.lastName].filter(Boolean).join(' ').trim()
    return data?.institutionName || fullName || data?.email || 'Unnamed client'
  }

  function clientSecondaryLabel({data}) {
    return [
      data?.clientNumber,
      data?.email,
      data?.phone,
  ...
  function money(value, currency = 'NAD') {
    const amount = Number(value || 0)
    return `${currency || 'NAD'} ${amount.toLocaleString(undefined, {
  ...
  function buildEngagemnetPayload(payload) {
    return {
      engagementCode: String(payload.engagementCode).trim(),
      clientId: String(payload.clientId).trim(),
      title: String(payload.title).trim(),
      serviceType: String(payload.serviceType).trim(),
      description: toTextOrNull(payload.description),
      studyLevel: toTextOrNull(payload.studyLevel),
      institutionName: toTextOrNull(payload.institutionName),
      assignedConsultantId: toTextOrNull(payload.assignedConsultantId),
      assignedTeam: toTextOrNull(payload.assignedTeam),
      priority: payload.priority || 'medium',
      status: payload.status || 'draft',
      deliveryStatus: payload.deliveryStatus || 'pending',
      satisfactionStatus: payload.satisfactionStatus || 'pending',
      ...
      currency: payload.currency || 'NAD',
      shareRuleId: toTextOrNull(payload.shareRuleId),
      ...
      remarks: toTextOrNull(payload.remarks),
      createdBy: payload.currentUserId || null,
    }
  }

  async function fetchClients(params = {pageSize:  100, sortBy: 'updatedAt'}) {
    try {
      return await fetchCollection(CRM_COLLECTIONS.clients, params);
    } catch (error) {
      throw normalizeError(error, 'Failed to load clients.');
    }
  }

  async function fetchRecentEngagements(params = {}) {
    try {
      return await fetchCollection(CRM_COLLECTIONS.engagements, params);
    } catch (error) {
      throw normalizeError(error, 'Failed to load engagements.');
    }
  }
  ...
  async function createEngagements(payload) {
    assertPermission('crm:write');
    try {
      const contact = await add(CRM_COLLECTIONS.engagements, buildEngagemnetPayload(payload));
      await createActivity({
        contactId: contact?.id || null,
        accountId: payload?.accountId || null,
        leadId: payload?.clientId || null,
        type: 'engagement_created',
        subject: 'Engagements created',
        description: `New Engagements ${buildFullName(payload?.title)} was added.`,
      });
      return contact;
    } catch (error) {
      throw normalizeError(error, 'Failed to create the contact.');
  ...
  async function fetchRecentFiles(params = {}) {
        /* await crmFilesActions.fetchInitialPage({
        pageSize: 20,
        sortBy: 'createdAt',
        sortDirection: 'desc',
      ...
      uploadedFiles.value = getCollectionItems('crm_files').filter(
        (entry) => entry?.clientId === form.clientId,
      ) */
    
    try {
      return await fetchCollection(CRM_COLLECTIONS.crmFiles, params);
    } catch (error) {
      throw normalizeError(error, 'Failed to load recent files.');
    }
  }
  ...
  return {
    ...
    updateContact, fetchAccounts,
    createAccount, updateAccount,
    fetchOpportunities, createOpportunity, moveOpportunityStage,
    fetchTasks, createTask, updateTask, completeTask,
    fetchActivities, createActivity, fetchNotes,
    createNote,
    fetchDocuments, createDocument, fetchMessages,
    logMessage, fetchAttachments, createAttachment,
    fetchSavedViews, createSavedView,
    fetchAutomationRules, createAutomationRule, fetchAssignmentRules,
    createAssignmentRule, convertLeadToOpportunity,
    fetchRecordTimeline, fetchLeadWorkspace, fetchCustomerRecordsSnapshot,
    searchEverything,
    fetchDashboardSnapshot,
    fetchReportsSnapshot,

    createEngagementCode, clientPrimaryLabel, money, asMoney,roundMoney, clientSecondaryLabel,
    formatDate, formatFileSize, buildEngagemnetPayload, fetchRecentEngagements, fetchRecentFiles,
    fetchClients, createEngagements
  };
}
