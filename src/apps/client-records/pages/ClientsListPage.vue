...
const stats = computed(() => {
  ...
  return [
    ...
    { label: 'Active', value: items.filter((item) => item.data.status === 'active').length },
    { label: 'Leads', value: items.filter((item) => item.data.status === 'lead').length },
    { label: 'Customers', value: items.filter((item) => item.data.lifecycleStage === 'customer').length },
  ]
})
