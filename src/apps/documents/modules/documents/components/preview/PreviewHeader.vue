<template>
  <header class="absolute grid grid-cols-[minmax(0,1fr)_auto] gap-4 border-b-2 pb-3" :style="headerStyle">
    <div class="flex items-start gap-4">
      <div class="flex h-14 w-14 items-center justify-center overflow-hidden rounded-[0.9rem] border bg-white/70">
        <img v-if="resolvedLogoUrl" :src="resolvedLogoUrl" alt="Logo" class="max-h-full max-w-full object-contain" @error="clearLogo" />
        <span v-else class="font-display text-lg font-bold" :style="{ color: document.brand.accentColor }">{{ initials }}</span>
      </div>

      <div>
        <div class="text-lg font-bold" :style="{ color: document.brand.primaryColor }">{{ document.brand.companyName }}</div>
        <div class="mt-1 whitespace-pre-line text-sm leading-6" :style="{ color: 'rgba(26, 26, 26, 0.68)' }">
          {{ document.layout.header.leftText || document.brand.legalLine }}
        </div>
      </div>
    </div>
    ...
    <div class="text-right">
      ...
      <div class="mt-2 text-lg font-bold" :style="{ color: '#1A1A1A' }">{{ document.meta.number || 'Draft' }}</div>
      <div class="mt-2 whitespace-pre-line text-xs leading-6" :style="{ color: 'rgba(26, 26, 26, 0.68)' }">
