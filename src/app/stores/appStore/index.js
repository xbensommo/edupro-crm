 ...
 * Permission checks are optional and only enforced when enabled in access config.
 *
 ...
 * Permission checks are disabled unless accessConfig.rbac.permissionsEnabled is true.
 *
 * @param {string[]} permissions
 * @returns {boolean}
 */
const hasAllPermissions = (permissions = []) => {
  ...
  const permissionsEnabled = Boolean(accessConfig?.rbac?.permissionsEnabled)
 ...
 * Permissions are enforced only when accessConfig.rbac.permissionsEnabled is true.
 *
...
const canAccessRoute = (routeMeta = {}) => {
  ...
  const permissionAllowed = hasAllPermissions(requiredPermissions)
