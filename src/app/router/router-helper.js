...
export function installEnterpriseRouterGuards(
  ...
  router.beforeEach(async (to, from, next) => {
    ...
    if (guestOnly && isAuthenticated) {
      ...
      return next(adapters.getDefaultDestination ? adapters.getDefaultDestination(store) : '/a')
    }

    const shouldCheckRbac = Boolean(accessConfig?.rbac?.enabled)

    if (shouldCheckRbac && typeof store.canAccessRoute === 'function') {
      const allowed = store.canAccessRoute(to.meta || {})
